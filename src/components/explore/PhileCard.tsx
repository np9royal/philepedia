...
export default function PhileCard({ phile, score, reason }: PhileCardProps) {
  ...
  return (
    <a
      href={`/${phile.slug}`}
      className="group relative flex h-full min-h-[18.5rem] flex-col overflow-hidden rounded-[1.75rem] border border-white/10 bg-white/[0.03] p-4 text-left shadow-[0_30px_90px_rgba(0,0,0,0.28)] transition duration-300 hover:-translate-y-1 hover:border-white/20 hover:bg-white/[0.05] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--phile-accent)] sm:p-5"
      style={style}
    >
      <div
      ...
      <div className="relative flex h-full flex-col">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[0.68rem] uppercase tracking-[0.3em] text-white/50">{phile.category}</p>
            <h3 className="mt-2.5 text-[1.55rem] font-semibold leading-tight text-white">{phile.name}</h3>
          </div>
          <div className="rounded-full border border-white/10 bg-black/25 px-3 py-1 text-[0.6rem] uppercase tracking-[0.28em] text-white/55">
            {phile.heroStyle}
          </div>
        </div>

        <p className="relative mt-3 text-[0.72rem] uppercase tracking-[0.24em] text-white/45">
          {phile.pronunciation}
        </p>

        <p className="relative mt-3 max-w-md text-sm leading-6 text-white/78">
        ...
        <div className="relative mt-4 flex flex-wrap gap-2">
          {phile.tags.slice(0, 4).map((tag) => (
            <span
              key={tag}
              className="rounded-full border border-white/10 bg-black/20 px-3 py-1 text-[0.66rem] uppercase tracking-[0.2em] text-white/60"
            >
              {tag}
            </span>
          ))}
